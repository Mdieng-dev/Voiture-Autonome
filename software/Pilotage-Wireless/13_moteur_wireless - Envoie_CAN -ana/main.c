...
void thread_motor(void const * arg) {
    Msg_t m;
    int vitesse_actuelle = 0, sens_actuel = 0;
		float vitesse_kmh = (float)vitesse_actuelle / VITESSE_MAX * V_MAX_REELLE_KMH;
    uint32_t angle_actuel = CENTRE;

    while (1) {
	
				// Lire l'état du GPIO0 (pin 23)
				//IA stop
        if ((LPC_GPIO0->FIOPIN2 & 0x80) == 0x80) {
        ...
        if (osMessageQueueGet(q_id, &m, NULL, 500) == osOK) {
            ...
            Action(vitesse_actuelle, sens_actuel, angle_actuel);
						donnees_moteur.vitesse = (int)vitesse_kmh; // En km/h entier
						donnees_moteur.angle = angle_actuel;
						osThreadFlagsSet(ID_CANthreadT, 0x01); // Notifie le thread CAN
...
void CANthreadT(void)
{
		...
		while (1) {
			...
			// Copie des données dans le buffer CAN
			data_buf[0] = (donnees_moteur.vitesse >> 8) & 0xFF; // Octet haut de la vitesse
			data_buf[1] = donnees_moteur.vitesse & 0xFF;        // Octet bas de la vitesse
			data_buf[2] = (donnees_moteur.angle >> 8) & 0xFF;  // Octet haut de l'angle
			data_buf[3] = donnees_moteur.angle & 0xFF;         // Octet bas de l'angle	
